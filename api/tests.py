...
class ApiTestCase(TestCase):
	def setUp(self):
		...
		with open(os.path.join(os.path.dirname(__file__), 'sampleCheck.json')) as json_file:
			json_data = json.load(json_file)

			correct_auth = f'Basic {os.getenv("KAMAR_AUTH_USERNAME")}/{os.getenv("KAMAR_AUTH_PASSWORD")}'

			self.good_req = self.client.post('/api/check/', json_data, content_type='application/json', **{'HTTP_AUTHORIZATION': correct_auth})
