...
from commendations import models as commendation_models
...
def generate_teacher(number):
	teacher_houses = [teacher_models.Teacher.ANDERSON, teacher_models.Teacher.BEGG, teacher_models.Teacher.HERRON, teacher_models.Teacher.ROSS, teacher_models.Teacher.SOMERVILLE]
...
def generate_student(number):
	student_houses = [student_models.Student.ANDERSON, student_models.Student.BEGG, student_models.Student.HERRON, student_models.Student.ROSS, student_models.Student.SOMERVILLE]
	for _ in range(number):
...
class Command(BaseCommand):
	...
	def handle(self, *args, **options):
		...
		self.stdout.write(self.style.NOTICE('Creating %s teachers...' % options['teachers'][0]))
		generate_teacher(options['teachers'][0])
		self.stdout.write(self.style.SUCCESS('Done!\n\n'))

		self.stdout.write(self.style.NOTICE('Creating %s students...' % options['students'][0]))
		generate_student(options['students'][0])
		self.stdout.write(self.style.SUCCESS('Done!\n\n'))

		self.stdout.write(self.style.SUCCESS('Successfully generated %s users' % (options['teachers'][0] + options['students'][0])))
